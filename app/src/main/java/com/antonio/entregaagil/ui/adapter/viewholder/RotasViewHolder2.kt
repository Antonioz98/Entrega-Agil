...
import kotlinx.android.synthetic.main.item_rotas2.view.*

class RotasViewHolder2(private val context: Context, itemView: View) : RecyclerView.ViewHolder(itemView) {

    fun binView(rota: Rota, clickListener: ((Rota, Int?) -> Unit)) {
        itemView.item_rota2_descricao.text = rota.descricao
        val cadastrados = context.getString(R.string.cadastrados)
        itemView.item_rota2_quantidade.text = "$cadastrados: ${rota.assinantes.size}/$ASSINANTES_POR_ROTA"
